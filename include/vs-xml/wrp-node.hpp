...
struct base_t{
    ...
    ///Construct a wrapper from its raw parts. Intended for low-level use and bindings.
    static base_t from(const TreeRaw& tree, const T* ptr){ return base_t(tree, ptr); }
...
struct base_t<attr_t>{
    ...
    ///Construct a wrapper from its raw parts. Intended for low-level use and bindings.
    static base_t from(const TreeRaw& tree, const attr_t* ptr){ return base_t(tree, ptr); }
 ...
 */
template <>
struct fmt::formatter<VS_XML_NS::wrp::sv, char> : fmt::formatter<std::string_view, char> {
    template <typename FormatContext>
    auto format(const VS_XML_NS::wrp::sv& value, FormatContext& ctx) const {
        return fmt::formatter<std::string_view, char>::format((std::string_view)(value), ctx);
    }
};