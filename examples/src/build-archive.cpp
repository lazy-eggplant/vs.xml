...
int main(){
    ArchiveBuilder<builder_config_t{.symbols=xml::builder_config_t::COMPRESS_ALL,.raw_strings=true}> bld;
    bld.reserve({1024*4,1024*4});

    constexpr std::string_view texts[] = {"Text 0", "Text 11", "Text 222"};

    fmt::print("Constructing frames.\n");
    ...
    for(auto text : texts){
        if(auto t = bld.document(text, [&](auto& bld){
            ...
            fmt::print(std::cerr,"Unable to complete construction of the tree {}",(int)t);
            exit(1);
        }
    }
    ...
    fmt::print("Saving as binary.\n");
    if(!archive.save_binary(memstream)){
        fmt::print(std::cerr, "Failed to properly save the binary file\n");
        exit(1);
    }
  
    memstream.flush();
    
    fmt::print("Loading from binary.\n");
    ...
    if(!archive_back.has_value()){
        fmt::print(std::cerr,"{}",archive_back.error().msg());
        exit(3);
    }

    fmt::print("Serializing documents to XML.\n");
